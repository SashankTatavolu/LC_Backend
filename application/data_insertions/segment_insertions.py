...
# def main():
#     file_path = "/home/sashank/Downloads/LC/Language_Communicator_Backend/application/data_insertions/Aug14_hindi/segments.txt"
#     chapter_id =85# Example: Use the specific chapter_id for the sentences
...
class Segment(Base):
    ...
    wx_text = Column(Text, nullable=True)
    segment_type = Column(String)
    index_type = Column(String)
    sentence = relationship("Sentence", back_populates="segments")
    chapter = relationship('Chapter', backref='segments')
...
def main():
    file_path = "/home/sashank/Downloads/LC/Language_Communicator_Backend/application/data_insertions/Aug14_2ch/segments.txt"
    chapter_id = 95 # Your chapter ID
    ...
    try:
        for line in lines:
            ...
            segment_index = parts[0].strip()
            segment_text = english_text = parts[1].strip()

            # Extract the base sentence_id (remove trailing letters if any)
            ...
            if sentence:
                new_segment = Segment(
                    ...
                    segment_text=segment_text,
                    english_text=english_text,
                    wx_text=None,  # Keep wx_text empty
                    segment_type="general",  # Default type
                    index_type="regular"     # Default index type
                )
                session.add(new_segment)
            else:
                print(f"Sentence with id {extracted_sentence_id} not found for segment {segment_index}")

        session.commit()
        print(f"Successfully inserted {len(lines)} segments")
...
# def main():
#     file_path = "/home/sashank/Downloads/LC/Language_Communicator_Backend/application/data_insertions/Ecommerce_data/health_data_part_2/segments.txt"
#     chapter_id = 47  # Example: Use the specific chapter_id for the sentences
...
#             if segment:
#                 # Update only the english_text and wx_text fields
#                 segment.english_text = english_text
#                 segment.wx_text = wx_text
#                 session.add(segment)
#             else:
#                 print(f"Segment with index {segment_index} not found.")

#         session.commit()
#         print("English and WX texts updated successfully!")

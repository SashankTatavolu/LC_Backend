...
from application.repository.verb import Verb
from application.repository.concept import Concept
from wxconv import WXC
import json
import tempfile
# from googletrans import Translator
...
def check_indeclinable(word_data):
    ...
    Functionality:
        ...
        2. Check if word is in INDECLINABLE_WORDS or UNITS list in application.repository.constant.py
    ...
    Result:
        True (as Aja exists in application.repository.constant.INDECLINABLE_WORDS)
...
def process_nouns(index_data,seman_data,nouns, words_info, verbs_data):
    ...
    for noun in nouns:
        ...
        if clean(noun[8]) in ('start','end','kriyAmUla','mod','whole','count'):
            case='d'
            postposition=None
        else:
